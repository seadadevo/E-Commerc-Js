...
.then(data => {
    ...
    const productContainerElectronics = document.getElementById('swiper_electronics'); 
    ...
    data.forEach((product) => {
        ...
        const percent_price = Math.floor(( product.old_price - product.price) / product.old_price * 100 );
    ...
    function displayProduct(container, condition) {
        data.forEach((product) => {
            if(product.category === condition) {
                const old_price_paragrapg = product.old_price ? ` <p class="old_price">$${product.old_price}</p>` : "" ;
                const percent_price = Math.floor(( product.old_price - product.price) / product.old_price * 100 );
                const percent_price_div = product.old_price ? ` <span class="sale_present">${percent_price}%</span>` : "" ;
